    ...
    def gen_pts_from_pdb(self, pdb_path, ntrans):
        ...
        for i in tqdm(range(1, len(pdb_files) + 1)):
            ...
            for _ in range(ntrans):
                ...
                log_file.write(str(pos_oxygen.shape[0]) + ' ' + file_name + '\n')
                ...
                for k in range(pos_oxygen.shape[0]):
                    xyz_file.write('O  ' + str(pos_oxygen[k, 0]) + ' ' + str(pos_oxygen[k, 1]) + ' ' + str(pos_oxygen[k, 2]) + '\n')
                xyz_file.close()
...
if __name__ == "__main__":
    ...
    if not os.path.exists('point_clouds/%s/xyz' % opt.category):
        os.makedirs('point_clouds/%s/xyz' % opt.category)
