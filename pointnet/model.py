...
class PointNetfeat(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(x.size()[0]):
            orig_mat = orig.cpu().detach().numpy()
            allPoints.append(orig_mat[i])
            crit_index_mat = crit_index.cpu().detach().numpy()
            critPoints.append(orig_mat[i][crit_index_mat[i]])
        allPoints = np.array(allPoints) #B*N*3
        critPoints = np.array(critPoints) #B*1024*3  
...
def normalize_point_cloud(batch_data):
    ...
    for k in range(batch_data.shape[0]):
        x_ave = np.mean(batch_data[k,:,0])
        y_ave = np.mean(batch_data[k,:,1])
        z_ave = np.mean(batch_data[k,:,2])
        batch_data[k,:,0] = batch_data[k,:,0] - x_ave
        batch_data[k,:,1] = batch_data[k,:,1] - y_ave
        batch_data[k,:,2] = batch_data[k,:,2] - z_ave
